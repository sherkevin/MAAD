...
class ICASSPAgent:
    ...
    def fit(self, X):
        """训练ICASSP智能体"""
        try:
            logger.info("训练ICASSP智能体: %s" % self.agent_id)
            
            # 高级信号预处理
            X_processed = self._signal_preprocessing(X)
            
            # 多尺度特征工程
            features = self._extract_icassp_features(X_processed)
            
            # 智能特征选择
            selected_features = self._intelligent_feature_selection(features)
            
            # 训练多模型集成
            self._train_icassp_models(selected_features)
            
            # 计算最优集成权重
            self._calculate_optimal_weights(selected_features)
            
            self.is_fitted = True
            logger.info("ICASSP智能体 %s 训练完成" % self.agent_id)
            
        except Exception as e:
            logger.error("ICASSP智能体 %s 训练失败: %s" % (self.agent_id, str(e)))
            self.is_fitted = False
    
    def _signal_preprocessing(self, X):
        """信号预处理 - 针对时间序列数据优化"""
        # 1. 处理NaN值
    ...
    def _extract_icassp_features(self, X):
        """提取ICASSP级别特征 - 结合信号处理和机器学习"""
        features = []
        ...
        if X.shape[1] > 4:
            ...
            # 频谱质心
            freq_centroid = np.sum(fft_features * np.arange(fft_features.shape[1]), axis=1) / np.sum(fft_features, axis=1)
        ...
        if X.shape[1] > 10:
            ...
            features.append(np.array(peak_features))
        ...
        if X.shape[1] > 5:
            ...
            features.append(np.array(autocorr_features))
    ...
    def _train_icassp_models(self, features):
        """训练ICASSP级别模型集成"""
    ...
    def predict(self, X):
        ...
        if not self.is_fitted:
            logger.warning("ICASSP智能体 %s 未训练，返回随机分数" % self.agent_id)
        ...
        try:
            ...
            logger.error("ICASSP智能体 %s 预测失败: %s" % (self.agent_id, str(e)))
            return np.random.rand(X.shape[0])

class ICASSPMultiAgentDetector:
    """ICASSP2026专用多智能体检测器"""
    ...
    def __init__(self, agents, config=None):
        ...
        logger.info("初始化ICASSP多智能体检测器，智能体数量: %d" % len(self.agents))
    
    def fit(self, X):
        """训练ICASSP多智能体检测器"""
        logger.info("开始训练ICASSP多智能体检测器，数据形状: %s" % str(X.shape))
    ...
    def predict(self, X):
        """ICASSP级别多智能体协作预测"""
        if not self.is_fitted:
            logger.warning("ICASSP多智能体检测器未训练，返回随机分数")
            return np.random.rand(X.shape[0])
        
        logger.info("开始ICASSP多智能体协作预测，数据形状: %s" % str(X.shape))
        ...
        # ICASSP级别融合策略
        logger.info("使用ICASSP级别融合策略进行智能体协作...")
        ...
        logger.info("ICASSP融合完成，最终分数范围: [%.4f, %.4f]" % 
...
def create_icassp_agents(config=None):
    """创建ICASSP智能体列表"""
    config = config or {}
    
    agents = [
        ICASSPAgent("icassp_trend_agent", "trend_analysis", config),
        ICASSPAgent("icassp_variance_agent", "variance_analysis", config),
        ICASSPAgent("icassp_residual_agent", "residual_analysis", config),
        ICASSPAgent("icassp_statistical_agent", "statistical_analysis", config),
        ICASSPAgent("icassp_frequency_agent", "frequency_analysis", config)
    ]
...
def test_icassp_system(dataset_name, data_base_path):
    """测试ICASSP多智能体系统"""
    logger.info("🚀 开始测试ICASSP多智能体系统 - %s数据集" % dataset_name)
    logger.info("=" * 60)
    ...
    # 测试ICASSP多智能体系统
    logger.info("🤖 测试ICASSP多智能体系统...")
    try:
        agents = create_icassp_agents()
        detector = ICASSPMultiAgentDetector(agents)
        ...
        icassp_results = {"ICASSPMultiAgent": {'auroc': auroc, 'f1': f1}}
        logger.info("ICASSPMultiAgent: AUROC %.4f, F1 %.4f" % (auroc, f1))
        
    except Exception as e:
        logger.error("ICASSP多智能体系统失败: %s" % str(e))
        icassp_results = {"ICASSPMultiAgent": {'auroc': 0.5, 'f1': 0.0}}
...
def main():
    ...
    logger.info("🚀 开始ICASSP2026多智能体异常检测系统测试")
    logger.info("=" * 80)
    
    # 测试MSL数据集
    msl_results = test_icassp_system("MSL", data_base_path)
    
    # 测试SMAP数据集
    smap_results = test_icassp_system("SMAP", data_base_path)
    
    # 保存结果
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    os.makedirs("outputs/icassp2026", exist_ok=True)
    ...
    results_file = "outputs/icassp2026/icassp_results_%s.json" % timestamp
    ...
    # 打印结果摘要
    logger.info("=" * 80)
    logger.info("📊 ICASSP2026多智能体系统测试结果摘要")
    logger.info("=" * 80)
    ...
    if "ICASSPMultiAgent" in avg_auroc:
        icassp_avg = np.mean(avg_auroc["ICASSPMultiAgent"])
        if icassp_avg > 0.7:
            logger.info("🎉 ICASSP多智能体系统达到顶级性能！符合ICASSP2026投稿要求！")
        elif icassp_avg > 0.6:
            logger.info("✅ ICASSP多智能体系统性能优秀，接近ICASSP2026投稿要求")
        else:
            logger.info("⚠️ ICASSP多智能体系统需要进一步优化以达到ICASSP2026投稿要求")
    
    logger.info("🎉 ICASSP2026多智能体系统测试完成！")
